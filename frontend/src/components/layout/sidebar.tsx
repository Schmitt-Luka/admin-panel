...
import { LayoutDashboard, LayoutGrid, Package, Receipt, LogOut, Store, ChevronLeft, ChevronRight, X } from 'lucide-react';
...
export function Sidebar() {
  ...
  const content = (
    <>
      <div className={cn('flex items-center gap-2 border-b px-5 py-4', collapsed && 'justify-center px-2')}>
        ...
        {!collapsed && <span className="text-base font-semibold">Admin Panel</span>}
        <button
          type="button"
      ...
      <div className="border-t p-3">
        <div className={cn('mb-2 flex items-center gap-2 rounded-md px-2 py-1.5', collapsed && 'justify-center px-0')}>
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'hidden h-screen shrink-0 flex-col border-r bg-card transition-all duration-200 md:flex',
          collapsed ? 'w-16' : 'w-60',
        )}
      >
        {content}
      ...
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-50 flex h-screen w-64 flex-col bg-card transition-transform duration-200 md:hidden',
